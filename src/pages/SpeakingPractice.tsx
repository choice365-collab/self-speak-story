...
function buildPhase3Instructions(verb: VerbData, difficultyLevel: string, speechSpeed: string): string {
  ...
  return [
    ...
    buildToneBlock(),
    "",
    `GOAL: Student practices a sentence BASED ON the learned sentences above, adapted to fit the situation. Do NOT invent unrelated sentences.`,
    "",
    "Each round — follow strictly:",
    "STEP 1 (Korean ONLY): Describe fun scenario in Korean. Ask '너라면 영어로 뭐라고 말할 것 같아?' WAIT.",
    `STEP 2 (English): Acknowledge what they said. Then pick ONE of the learned sentences above and adapt it to the scenario as a hint — keep the core structure but adjust details to fit the situation. Leave only 1-2 words blank for the student to fill in. The hint must closely resemble one of the learned sentences. WAIT for student to try completing it.`,
